    ...
    def what_if_analysis(
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                # Note: what-if may return empty output or non-JSON in some cases
                data = {}
                if result.stdout and result.stdout.strip():
                    try:
                        data = json.loads(result.stdout)
                    except json.JSONDecodeError:
                        # what-if sometimes outputs formatted text instead of JSON
                        # Log but continue - validation already passed
                        console.print(
                            "[yellow]Note: What-if output was not JSON formatted[/yellow]"
                        )
